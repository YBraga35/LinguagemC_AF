...
void lancamento(int *n)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        printf("\t- Frequência (número de 0 a 100, separado por vígula): ");
        scanf("%f", &estudantes[*n].frequencia);
        printf("\tDigite o valor das 3 notas do estudantes, de 0 a 10, separado por vírgula: \n");
        for (int j = 0; j < 3; j++)
        {
            printf("\n- Nota %d: ", j+1);
    ...
    for (int k= (grav - 1); k < est; k++)
    {
        ...
        if (haerro < 0)
        {
            printf("Erro na gravação em arquivo no Lançamento L130");
            system("pause");
        }
        printf("Fazendo a gravação em arquivo no Lançamento L135");
    }
    printf("Fazendo a gravação em arquivo no Lançamento L137");
    fclose(lanca);
    system("pause");
}
void consulta()
