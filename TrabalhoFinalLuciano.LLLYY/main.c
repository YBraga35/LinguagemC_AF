...
int contaAlunos()
{
    ...
    int k = 0;
    while (fscanf(contagem,"%d; %[^;];%f;%f;%f;%f;%f;%s\n", &estudantes[k].matricula, estudantes[k].nome, 
                    &estudantes[k].frequencia, &estudantes[k].notas[0], &estudantes[k].notas[1], 
                    &estudantes[k].notas[2], &estudantes[k].media, estudantes[k].resultado)!= EOF)
    {
        k++;
    }
    fclose(contagem);
    ...
    if (controleRelat == 0)
    {
        ...
        for (int k = *estInic; k < est; k++)
        {
            if (est != *estInic)
            {
                ...
                int haerro = fprintf(lanca, "%d;%s;%.2f;%.2f;%.2f;%.2f;%.2f;%s\n", estudantes[k].matricula, 
                                        estudantes[k].nome, estudantes[k].frequencia, estudantes[k].notas[0], 
                                        estudantes[k].notas[1], estudantes[k].notas[2], 0.0, "x");
                if (haerro < 0)
                {
                                    ...
                                    &estudantes[k].notas[2], &estudantes[k].media, estudantes[k].resultado)!= EOF)
    {
        ...
        if (controle != 0)
        {
            printf("%-10d %-30s %-15.2f %-10.2f %-10.2f %-10.2f %-10.2f %-10s\n", estudantes[k].matricula, 
                        estudantes[k].nome, estudantes[k].frequencia, estudantes[k].notas[0], estudantes[k].notas[1], 
                        estudantes[k].notas[2], estudantes[k].media, estudantes[k].resultado);
        }
        k++;
    }
